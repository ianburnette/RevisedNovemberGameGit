...
public class CharacterMotor : MonoBehaviour 
{
    ...
    public void RotateToNormal(Vector3 normal, float turnSpeed, bool ignoreY)
    {
        ...
        Vector3 newDir = normal;//new Vector3(-normal.z, 0, -normal.x);
        Quaternion dirQ = Quaternion.LookRotation(-newDir, Vector3.up);
        Quaternion slerp = Quaternion.Slerp(transform.rotation, dirQ, turnSpeed * Time.deltaTime);
        debugText.text = "target normal is " + normal + " and transform.forward is " + transform.forward + " and currentRotation is " 
            + transform.rotation.eulerAngles + " and target is " + slerp.eulerAngles;
        rigid.MoveRotation(slerp);
    }
