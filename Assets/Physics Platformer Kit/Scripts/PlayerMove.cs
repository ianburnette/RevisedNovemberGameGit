...
public class PlayerMove : MonoBehaviour 
{
	...
	public PlayerAnimation animator;			//object with animation controller on, which you want to animate
	...
	void FixedUpdate() 
	{
        ...
        if (ledgeHandling.LedgeMovementFilter(ref slopeMovementVector))
        {
            MaintainHeight(ledgeHandling.LedgeHeight);
        }
        else if (!rigid.useGravity)
        {
         //   print("using gravity again");
        ...
        if (rotateSpeed != 0 && direction.magnitude != 0 && !ledgeHandling.OnLedge)
            characterMotor.RotateToDirection(moveDirection, curRotateSpeed * 5, true);
        else if (ledgeHandling.OnLedge)
        {
            characterMotor.RotateToNormal(ledgeHandling.VerticalPlaneNormal, curRotateSpeed * 10f, true);
            //characterMotor.RotateToDirection(-ledgeHandling.VerticalPlaneNormal, curRotateSpeed * 5, true);
            //transform.position = hit.point;
           // Vector3 adaptedPlaneNormal = new Vector3(ledgeHandling.VerticalPlaneNormal.x, 0, ledgeHandling.VerticalPlaneNormal.z);
           // var q = Quaternion.FromToRotation(transform.up, adaptedPlaneNormal);
           // transform.rotation = Quaternion.Lerp(transform.rotation, q, curRotateSpeed);

        }
        //   characterMotor.RotateToDirection(ledgeHandling.VerticalPlaneHit, curRotateSpeed, true);
    ...
    void MaintainHeight(float ledgePos)
    {
        if (rigid.useGravity)
        {
        ...
        transform.position = Vector3.Lerp (transform.position, new Vector3(transform.position.x, ledgePos + (ledgeHandling.HangOffset), transform.position.z), heightCorrectionSpeed * Time.deltaTime);
      //  curRotateSpeed = 0;
    }
